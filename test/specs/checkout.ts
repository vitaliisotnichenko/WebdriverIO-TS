...
describe('Cart', () => {
    it('can add item', function () {
        ...
        checkout.open();
        expect(checkout.isItemInCart()).toBeDisplayed()
    })
    ...
    it.only('can add correct item', async function () {
        ...
        expect(productNameInCart).to.equal((await productDetails).name);
        expect(productPrice).to.equal((await productDetails).price);
    })
})
